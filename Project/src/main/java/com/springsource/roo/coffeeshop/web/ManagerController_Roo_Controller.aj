...
import com.springsource.roo.coffeeshop.web.ManagerController;
...
privileged aspect ManagerController_Roo_Controller {
    ...
    public String ManagerController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("managers", Manager.findManagerEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) Manager.countManagers() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("managers", Manager.findAllManagers(sortFieldName, sortOrder));
        }
